...
// key expand core
void KeyExpansionCore(byte *in, int i);
// key expansion
...
void KeyExpansionCore(byte in[4], int i)
{
    // dịch trái 1 bytes
    byte t = in[0];
    in[0] = in[1];
...
void aes_decrypt_CBC_mode(const string &cipher, string &recovered, const byte *expandedKeys, const int &keysize, const byte *iv)
{
    ...
    if (nThread)
    {
        ...
        aes_decrypt_CBC_core((byte *)&recovered[nThread << 12], (byte *)&cipher[nThread << 12], expandedKeys, keysize, iv, n);
